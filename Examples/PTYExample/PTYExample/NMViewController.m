...
- (void)viewDidLoad {
    ...
    self.portField.text = [[NSUserDefaults standardUserDefaults] stringForKey:@"port"];
...
- (IBAction)authentication:(id)sender {
    self.passwordField.enabled = !(self.authenticationControl.selectedSegmentIndex == 1);
}

- (IBAction)login:(id)sender {
    if (self.hostField.text.length == 0 || self.portField.text.length == 0 || self.usernameField.text.length == 0 || (!(self.authenticationControl.selectedSegmentIndex==1) && self.passwordField.text.length == 0)) {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([[segue identifier] isEqualToString:@"loginSegue"]) {
        ...
        terminalController.port = self.portField.text;
        terminalController.username = self.usernameField.text;
        terminalController.selectedSegmentIndex = self.authenticationControl.selectedSegmentIndex;
        

        if (self.authenticationControl.selectedSegmentIndex != 1) {
